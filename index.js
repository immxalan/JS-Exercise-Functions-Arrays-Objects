 ...
 * }
*/
function makePersonObject(idInput, nameInput, emailInput) {
const PersonObject = {
  id: idInput,
  name: nameInput,
  email: emailInput,
...
console.log(makePersonObject("5", "allen", "immxalan@gmail.com"))
/**
 * ### Challenge `getName`
 * 
 * @instructions
...
console.log(getName(nameObject))
...
console.log(makeSmartPerson("Eric"))
...
*/
// const carInventory = require("./inventory");//output: [..... from inventory.js]
const carInventory = require("./inventory");//output: [..... from inventory.js]
function getLastCarInfo(carInventory) {
  const lastCar = carInventory.find((item, index) => {return index === 49})
  return `This is a ${lastCar.car_make} ${lastCar.car_model}`
}
console.log(getLastCarInfo(carInventory))
...
function getCarInfoById(carInventory ,carid) {
  const carIndex = carInventory.findIndex(x=>x.id===carid)
  return `This is a ${carInventory[carIndex].car_make} ${carInventory[carIndex].car_model}`
}
console.log(getCarInfoById(carInventory, 1))
/**
...
return carInventory.sort((a,b) => (a.car_model > b.car_model) ? 1 : ((b.car_model > a.car_model) ? -1:0))
}
...
*/
function getModelYears(carInventory) {
const carYear = carInventory.map(x => x.car_year)
return carYear
}
...
*/
function getOlderCars(carInventory, maxYear) {
const OlderCars = carInventory.filter(function(item){
  return item.car_year <= maxYear})
  return OlderCars
}
 ...
 * in the same order as they appear in the original inventory.
*/
function getGermanCars(carInventory) {
 const filter1 ="Audi"
 const filter2="Mercedes-Benz"
 const filter3="Volkswagen"
 const filter4="BMW"
 const filter =[filter1, filter2, filter3, filter4]
  const germanCars = carInventory.filter (({car_make}) => filter.includes(car_make))
    return germanCars
}
console.log(getGermanCars(carInventory))
/**
...
*/
function carMaker(odometerInput) {
const Object={
    odometer: odometerInput,
    drive(distance) {
       return this.odometer = odometerInput + distance;}}
return Object}
